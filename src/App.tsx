...
function App() {
  ...
  return (
      <div className="App flex flex-col justify-center gap-12 max-w-4xl w-full p-8">
          ...
          <p>
              ¿Quieres experimentar como es leer a una velocidad determinada en japonés? ¡Esta es tu página! Puedes
              modificar el texto, la velocidad de lectura y al darle a "Iniciar", las palabras empezarán a aparecer a la
              velocidad que hayas seleccionado. Puedes pausar y reiniciar cuando quieras.
          </p>
          <p className="text-4xl flex justify-center items-center gap-8 text-center relative">
              <span className="absolute -translate-x-[200px] text-gray-400 text-[1.5rem]">{previousWord}</span>
              <span className="font-bold">{currentWord}</span>
              <span className="absolute translate-x-[200px] text-gray-400 text-[1.5rem]">{nextWord}</span>
          </p>
          <div className="flex flex-col items-center gap-4">
              <p className="text-2xl">Velocidad de lectura</p>
              <Slider defaultValue={[20000]} max={50000} min={1000} step={1000}
                      onValueChange={(value) => setCharactersPerHour(value[0])}/>
          ...
          <Accordion type="single" collapsible className="text-white">
              <AccordionItem value="item-1">
                  ...
                  <AccordionContent>
                      <Textarea className="w-full h-64 text-black" value={text}
